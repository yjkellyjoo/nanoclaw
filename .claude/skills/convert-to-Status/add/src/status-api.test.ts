...
describe('status-api', () => {
  ...
  describe('createAccount', () => {
    it('POSTs expected payload and extracts keyUID', async () => {
      ...
      await expect(api.createAccount(8080, 'Test User', 'secret')).resolves.toEqual({
        keyUID: 'key-123',
      });
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://127.0.0.1:8080/statusgo/CreateAccount',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ displayName: 'Test User', password: 'secret' }),
        },
      );
    });
    ...
    it('throws when response does not include keyUID', async () => {
      ...
      await expect(api.createAccount(8080, 'Test User', 'secret')).rejects.toThrow(
